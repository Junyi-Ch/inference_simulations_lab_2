...
Two sample t-test for tea_data: t = 0.51261, df = 16, p-value = 0.6152

Two sample t-test for tea_data_highn: t = 2.1329, df = 46, p-value = 0.0383
...
```{r}
tea_data_summary <- samps |> #save samps to tea_data_summary
  group_by(sim, condition) |>  #group by sim and condition
  summarise(mean_rating = mean(rating)) |> #summarize the mean ratings and save to mean_rating
  group_by(sim) |> #only group by sim
  summarise(delta = mean_rating[condition == "milk first"] -
              mean_rating[condition == "tea first"]) #calculate the difference between the means for each condition
```
...
The higher-m one is more normally distributed, with more counts on lower delta values.
...
Sim is each simulation, and each participant has 1000 of them with the same "sim".
...
There data are more concentrated in the middle, and there are fewer extreme values.
...
Each dot represents a participant. The range represents the range of the ratings for all the participants. The confidence interval indicates there's a 95% chance that the true rating is within this range. The SE indicates how likely the population mean is from the sample mean.

To do: how does this change when you use the low-n experiment?

The confidence interval and SE become larger.
...
```{r}
all_results=tibble() #create an all-results dataset
